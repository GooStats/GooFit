...
__host__ double GooPdf::evaluateAtPoints(const std::vector<double> &v) {
  copyParams();
  normalise();
  //  MEMCPY_TO_SYMBOL(normalisationFactors, host_normalisation, totalParams*sizeof(fptype), 0, cudaMemcpyHostToDevice);
  UnbinnedDataSet tempdata(observables);
  ...
  thrust::host_vector<fptype> h_results = results;
  return h_results[0] * host_normalisation[parameters];
}
...
__host__ void GooPdf::debug() const {
  ...
  std::cout << getName() << "  initial condition: id";
...
bool GooPdf::IsChisquareFit() const { return fitControl->getMetric() == "ptr_to_Chisq"; }
MetricTaker::MetricTaker(PdfBase *dat, void *dev_functionPtr)
    : metricIndex(0), functionIdx(dat->getFunctionIndex()), parameters(dat->getParameterIndex()) {
